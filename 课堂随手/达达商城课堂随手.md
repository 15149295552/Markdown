...
```mysql
1.确认微博表中数据(字段:user_profile_id 为 NULL)
  select * from users_weibo_profile;
2.查询zhaoliying用户的id(我的id为6)
  select id,username from users_user_profile where username="zhaoliying";
3.更新微博表外键
  update users_weibo_profile set user_profile_id=6;
4.确认是否更新正确(确认外键已经为6，不为 NULL了)
  select * from users_weibo_profile;
```

* 使用微博扫码登录，会直接跳转到zhaoliying的主页
* 使用正式用户登录，也会跳转到zhaoliying的主页
